...
public class RankHighlightStep extends ColorIndexesTwoColors implements RankStep{
    
        private int index1,index2;
        private int [] ranks, sortedranks;
        private byte [] values, values2, sortedColors;

	public RankHighlightStep(byte[] values, byte [] values2,
                int [] ranks, int [] sortedranks, int index1, int index2) {
		super(PaintTimes.RANK_SWAP, values, 
                        Colors.BAR_RANK_HIGHLIGHT, Colors.BAR_RANK_REMOVE, 
                                index1, index2);
		this.index1 = index1;
                this.index2 = index2;
                this.values = values;
