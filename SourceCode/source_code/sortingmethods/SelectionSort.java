...
import steps.array.selection.SelectionSmallestStep;
...
public class SelectionSort extends SortingMethod{
    ...
    public Step[][] solve(int[] initialValues) {
      ...
      int smallest; //keeps track of smallest value of array
        ...
        for (int pass=0; pass < (values.length-1); pass++) {
           ...
           */
            ...
            for (int j = pass + 1; j < values.length; j++) {
                 ...
                 * are passed into the SelectionCompareStep class. Here the 
                 ...
                 * Steps.
                 */
                currentSteps.add(new SelectionCompareStep(values.clone(), smallest, j));
                
                /* Swap Step:
                 * If a swap is needed, the SelectionSwapStep class will be created
                ...
                if (values[smallest] > values[j]) {
                    smallest = j;
                    currentSteps.add(new SelectionSmallestStep(values.clone(), smallest));
                }
 
            }
                byte temp = values[pass];
                values[pass] = values[smallest];
                values[smallest] = temp;
                
                currentSteps.add(new SelectionSwapStep(values.clone(), smallest, pass));
   ...
   public static void main(String[] args) {
        ...
        */
      int[] unsorted = new int[] { 30, 12, 21, 23, 42, 28, 14, 30, 37, 30, 25,
           47, 24, 26, 10}; 
       ...
       */
      Step[][] steps = (new SelectionSort()).solve(unsorted);
